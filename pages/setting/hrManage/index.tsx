...
import {
  ...
  apiPostDepartments_id_managers,
  apiDeleteDepartments_id_managers,
...
export default function HrManage() {
  ...
  // 刪除功能
  // 確定刪除
  const delMenager = async ({ departmentId, employeeId }: { departmentId: string; employeeId: string }) => {
                ...
                {employeesArr?.map((emp) => {
                  ...
                  const jobsStrArr =
                    jobs?.map((job) => {
                      ...
                      if (code) {
                        str = `${code} / ${departmentName} / ${jobName} / Level ${grade}`;
                      }
