...
export default function MemoList() {
  ...
  const { data: annotationArr, reset, viewRef, isLoading: getIsLoading } = useGetAnnotation_v2(params);
  ...
  const apiReq = async (method: 'post' | 'patch' | 'delete', delId?: string) => {
    ...
    if (method !== 'delete') {
      ...
      const { id: classId, apiBody } = classAnnotation;
    ...
    try {
      ...
      const apiReq = (() => {
        if (method === 'post') {
          return () => apiPostAnnotation({ body: body! });
        }

        if (method === 'patch') {
          return () => apiPatchAnnotation({ body: body!, id: id! });
        }

        if (method === 'delete') {
          return () => apiDeleteAnnotation({ id: delId! });
        }
      })();
      await apiReq?.();
      await reset();
  ...
  return (
    <SubLayer className={scss.container} bodyClassName={classNames(scss.subLayer, scss.plus)}>
      ...
      <Table_annotation annotationArr={annotationArr} hookPack={hookPack} apiReq={apiReq} viewRef={viewRef} />
...
export class Class_annotation {
  ...
  get category() {
    return this._annotation.category;
  }
  set category(v) {
    this._annotation.category = v;
    this._reRender();
  }

  get doorModelName() {
    return this._annotation.doorModelName;
  }
  set doorModelName(v) {
    this._annotation.doorModelName = v;
    this._reRender();
  }
  ...
  set description(v) {
    this._annotation.description = v;
    this._reRender();
  }
