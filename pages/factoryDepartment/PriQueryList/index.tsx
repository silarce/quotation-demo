  ...
  const filterData = () => {
    ...
    if (type) {
      filteredData = filteredData.filter(
        (item) => item.detail_type?.toString().toLowerCase().includes(type) // 忽略大小寫
      );
    }
    ...
    if (supplier) {
      filteredData = filteredData.filter(
        (item) => item.detail_suppliername?.toString().toLowerCase().includes(supplier) // 忽略大小寫
      );
    }

    if (productid) {
      filteredData = filteredData.filter(
        (item) => item.detail_productid?.toString().toLowerCase().includes(productid) // 忽略大小寫
      );
    }

    if (name) {
      filteredData = filteredData.filter(
        (item) => item.detail_name?.toString().toLowerCase().includes(name) // 忽略大小寫
      );
    }

    if (spec) {
      filteredData = filteredData.filter(
        (item) => item.detail_spec?.toString().toLowerCase().includes(spec) // 忽略大小寫
      );
    }
    ...
    <SubLayer isLoading_subLayer={false}>
      ...
      >
        <div
          ...
          style={{ overflowX: 'auto', maxHeight: `${isCollapsed ? '443px' : '660px'}` }}
        >
          {/* <Thead01 type={'PEntry'} /> */}
          <div className={scss.head15}>
            <span>
              <button onClick={() => setIsCollapsed(!isCollapsed)} style={{ paddingTop: '10px' }}>
                {isCollapsed ? (
              ...
              return (
                    ...
                    myAlert.confirm({
                      ...
                      props: {
                        onOk: () => {
                          ...
                          router.push({
                            pathname: `${
                              updatedItem.detail_type === '詢價'
                                ? '/factoryDepartment/QReqDetail'
                                : '/factoryDepartment/PReceiptDetail'
                            }`,
                            query: {
                ...
                >
                  <>
                    ...
                    >
                      <span style={{ color: '#14256a' }}>{_item.detail_type}</span>
                      <span>{index + 1}</span>
