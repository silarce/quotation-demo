...
export default function PRequisitionList() {
  ...
  const filterData = () => {
    ...
    // 檢查是否所有條件都為空
    if ((!startDate || !startDate.isValid()) && (!endDate || !endDate.isValid()) && !id && !status && !note) {
      setSearchdata(data);
    ...
    // 模糊查詢請購單號
    if (id) {
      filteredData = filteredData.filter((item) => item.purchaserequisitionid.toString().includes(id));
    }
    ...
    if (note) {
      filteredData = filteredData.filter((item) => item.note.toString().includes(note));
    }
                            ...
                            <tbody>
                              {details[_item.purchaserequisitionuuid]?.map((detail: any, detailIndex: number) => (
                                <tr key={detailIndex}>
                                  ...
                                  <td style={{ width: '150px', color: '#ea1833' }}>{detail.po_quantity ?? '0'}</td>
