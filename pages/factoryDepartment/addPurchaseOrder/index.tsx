...
import icon_history from 'public/image/icon/fc_history.svg?url';
...
export default function AddPurchaseOrder() {
  ...
  useEffect(() => {
    ...
    setNeed_date(dayjs().format('YYYY-MM-DD') || '');
  }, []);
  ...
  const getProductById = async (productuuid: any) => {
    try {
      ...
      setData2((prevData2) => {
        ...
        const duplicateProductIds = data.filter((item: { productid: any }) =>
          existingProductIds.includes(item.productid)
        );

        if (duplicateProductIds.length > 0) {
          myAlert.info({
            title: `${duplicateProductIds.map((item: { productid: any }) => item.productid).join(', ')}已加入`,
          });
        }
  ...
  const handlecancelAddPR = () => {
    if (editmain === true) {
      ...
      setData2([]);
      setNote('');
      setNeed_date(dayjs().toString());
    }
  };

  const handlesaveAddDetail = () => {
    const needDateObj = new Date(need_date);
    const createAtinObj = new Date(create_atin);
  ...
  const filterData = () => {
    ...
    // 檢查是否所有條件都為空
    if ((!startDate || !startDate.isValid()) && (!endDate || !endDate.isValid()) && !requisitionId && !status) {
    ...
    // 模糊查詢單號
    if (requisitionId) {
      filteredData = filteredData.filter((item) => item?.purchaseorderid?.toString().includes(requisitionId));
    }
  ...
  useEffect(() => {
    if (customersisSelectingRef.current) {
      return;
    }
    ...
    if (supnameFilter !== '' || supaddressFilter !== '') {
      const filtered = customerdata.filter((item) => {
        ...
        const matchesAddress =
          (item.county && item.county?.toLowerCase().includes(supaddressFilter)) ||
  ...
  const handleSelectCustomer = (item: any) => {
    ...
    setSupplieraddressin(
      (item.county ? item.county : '') + (item.district ? item.district : '') + (item.address ? item.address : '')
    );
  ...
  useEffect(() => {
    if (shippingisSelectingRef.current) {
      return;
    }
  ...
  const handleCheckboxChange = (item: any) => {
    ...
    setHandinputtotalprice(
      (parseFloat(item.detail_unitprice || '0') * parseFloat(handinputquantity || '0')).toString()
    );
      ...
      <div className={scss.container} style={{ height: `${windowSize.height - 198}px` }}>
          ...
          <div className={scss.content}>
            <div className={scss.head_head1}>
              ...
              <div>
                ...
                >
                  <img
                    src={status === '未儲存' || editmain === true ? icon_cancel.src : icon_cancel_gray.src}
                    alt="search"
                    style={{ height: '20px', width: '20px' }}
                  />
                  取消
              ...
              <div>
                <button
                  style={{ display: `${status === '未送出' ? '' : 'none'}` }}
                  className={scss.redsquarebtn}
                  onClick={() => {
                    handleDeletePO(purchaseorderuuid);
                  }}
                  title="單據刪除"
                >
                ...
                <div className={scss.head_content1}>
                  ...
                  <div style={{ paddingRight: '20px' }}>
                    <InputSel
                      ...
                      datePickerProps={{
                        props: {
                          value: need_date ? dayjs(need_date) : null,
                          onChange: (e) => {
                            setNeed_date(e ? dayjs(e).format('YYYY-MM-DDTHH:mm:ssZ') : '');
                          },
                        },
                      }}
                    />
            ...
            <div className={scss.body_content1} style={{ overflowX: 'auto' }}>
                ...
                <CellWithBar key={index} className={scss.panelHeader20}>
                  <div className={scss.row01}>
                    <span>
                      ...
                      <button
                        style={{ display: editstatus === false ? '' : 'none' }}
                        onClick={() => {
              ...
              <div
                className={scss.addbar}
                style={{
                  display: `${purchaseorderid != '' && status === '未送出' ? '' : 'none'}`,
              ...
              >
                ...
                <div>
                  <input type="text" placeholder="請輸入品項規格" value={handinputspec} onChange={handleSpecChange} />
                </div>
                <div>
                  <input
                  ...
                  <CellWithBar key={index} className={scss.panelHeader15}>
                    ...
                    >
                      ...
                      <span
                        style={{
                          color:
                            _item.status === '已結案' ? '#14256a' : _item.status === '詢價中' ? '#28a745' : '#ea1833',
                        }}
                      >
                        {_item.status}
                      </span>
        ...
        >
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '16px', width: '920px', padding: '10px 15px' }}>
            ...
            <span style={{ fontSize: '16px' }}>{quotereqname}</span>&nbsp;&nbsp;&nbsp;&nbsp;
            <span style={{ fontSize: '16px', color: '#14256a' }}>規格：</span>
            <span style={{ fontSize: '16px' }}>{quotereqspec}</span>&nbsp;&nbsp;&nbsp;&nbsp;
            <span style={{ fontSize: '16px', color: '#14256a' }}>數量：</span>
            <span style={{ fontSize: '16px' }}>{quotereqquantity}</span>
          </div>
          <hr />
          {/* {selectedsupplier} */}
          <div>
