...
export default function TrayDetail() {
  ...
  const GetWhp = async (id: any, trayname: any, whid: any) => {
    try {
      ...
      setwhpid(
        recodeWhpid(responseData?.length, responseData?.width, responseData?.childlength, responseData?.childwidth)
      );
  ...
  // 根據篩選條件更新資料
  useEffect(() => {
    const filtered = searchbardata.filter(
      (item) =>
  ...
  return (
    <SubLayer isLoading_subLayer={false} className="overflow-hidden">
      <PageHeader02
        tag={`倉庫編號：${whname}｜托盤編號：${trayname}`}
        panelList={undefined}
        customeLeft={[<>{/* 托盤呼叫狀態:{called?.toString()} */}</>]}
        customeRight={[
          <>
            {isEditing ? (
                  ...
                  <button
                    ...
                    onMouseEnter={(e) => {
                      (e.target as HTMLButtonElement).style.backgroundColor = `${
                        called ? 'rgb(0, 105, 23, 0.267)' : '#ea18341d'
                      }`;
                    }}
                    onMouseLeave={(e) => {
