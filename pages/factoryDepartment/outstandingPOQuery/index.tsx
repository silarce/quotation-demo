...
import scss from './outstandingPOQuery.module.scss';
  ...
  const filterData = () => {
    ...
    if (supplierid) {
      filteredData = filteredData.filter(
        (item) => item.supplierid?.toString().toLowerCase().includes(supplierid) // 忽略大小寫
      );
    }
    ...
    if (supplier) {
      filteredData = filteredData.filter(
        (item) => item.suppliername?.toString().toLowerCase().includes(supplier) // 忽略大小寫
      );
    }

    if (productid) {
      filteredData = filteredData.filter(
        (item) => item.productid?.toString().toLowerCase().includes(productid) // 忽略大小寫
      );
    }

    if (name) {
      filteredData = filteredData.filter(
        (item) => item.name?.toString().toLowerCase().includes(name) // 忽略大小寫
      );
    }

    if (spec) {
      filteredData = filteredData.filter(
        (item) => item.spec?.toString().toLowerCase().includes(spec) // 忽略大小寫
      );
    }
                  ...
                  <>
                    ...
                    >
                      ...
                      <span style={{ color: '#ea1833' }}>
                        {(Number(_item.quantity) - Number(_item.alreadyinquantity)).toLocaleString()}
                      </span>
