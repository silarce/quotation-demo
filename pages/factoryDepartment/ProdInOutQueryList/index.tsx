...
export default function ProdInOutQueryList() {
  ...
  const filterData = () => {
    ...
    if (productid) {
      filteredData = filteredData.filter((item) => item.detail_productid?.toString().toLowerCase().includes(productid));
    }

    if (name) {
      filteredData = filteredData.filter((item) => item.detail_name?.toString().toLowerCase().includes(name));
    }

    if (spec) {
      filteredData = filteredData.filter((item) => item.detail_spec?.toString().toLowerCase().includes(spec));
    }
                    ...
                    myAlert.confirm({
                      ...
                      props: {
                        onOk: () => {
                          router.push({
                            pathname: `${
                              _item.detail_type === '出庫'
                                ? '/factoryDepartment/PKingDetail'
                                : '/factoryDepartment/PEntryDetail'
                            }`,
                            query: {
