...
export default function AddProdReceipt() {
  ...
  const handleToggleProdreceiptdetail = async (item: any, isChecked: boolean) => {
    ...
    if (
      suppliernamein === '' ||
      suppliernamein === null ||
      suppliernamein === undefined ||
      suppliernamein === item.purchaseOrder.suppliername
    ) {
        ...
        if (isChecked) {
          ...
          setData2((prevData2) => {
            const newDetails = item.details.filter(
              (detail: any) => !prevData2.some((existingDetail) => existingDetail.id === detail.id)
            );
          ...
          setData2((prevData2) => {
            const updatedData2 = prevData2.filter(
              (existingDetail: any) => !item.details.some((detail: any) => detail.id === existingDetail.id)
            );
  ...
  const filterData = () => {
    ...
    // 模糊查詢請購單號
    if (requisitionId) {
      filteredData = filteredData.filter((item) => item.prodreceiptid.toString().includes(requisitionId));
    }
    ...
    if (suppliername) {
      filteredData = filteredData.filter((item) => item.suppliername.toString().includes(suppliername));
    }
  ...
  return (
    <SubLayer isLoading_subLayer={isLoading}>
      <PageHeader02 tag={'新增進貨單'} panelList={panelList} customeLeft={[]} />
                    ...
                    data2.map((_item: any, index: number) => (
                      <CellWithBar key={index} className={scss.panelHeader14}>
                        <div className={scss.row01}>
                          ...
                          <span className="truncate" title={_item.note}>
                            {_item.note}
                          </span>
                        </div>
                      </CellWithBar>
