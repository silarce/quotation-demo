...
import { Toption, optionsCreator_doorRail_normal, optionsCreator_doorRail_antyTyphoon } from 'js/utils/options/options';
...
class Class_mainProduct {
  constructor(mainProduct: Tdata['mainProductArr'][0], reRender: TreRender) {
    this._reRender = reRender;
    ...
    this._B = (() => {
      if (mainProduct.B) {
        return unexpectedOption(`${mainProduct.B}`);
      } else {
        return { value: 'autoCalc', label: '自動計算' };
      }
    })();
    ...
    this._horsepower = (() => {
      if (mainProduct.horsepower) {
        return unexpectedOption(mainProduct.horsepower);
      } else {
        return { value: 'autoCalc', label: '自動計算' };
      }
    })();
    ...
    this._partArr = mainProduct.part.map((partItem) => {
  ...
  private _unitWeight;
  get weight() {
    return new Decimal(this._L || 0)
      .mul(this._h || 0)
      .mul(this._unitWeight || 0)
      .toString();
  }
  ...
  get BOption(): Toption[] {
    ...
    if (optionArr[0]) {
      optionArr.unshift({ value: 'autoCalc', label: '自動計算' });
    }
  ...
  get ejectionDoor() {
    return this._ejectionDoor;
  }
  set ejectionDoor(v: boolean) {
    this._ejectionDoor = v;
    this._reRender();
  }

  private _doorRail;
  get doorRail() {
    return this._doorRail;
  }
  set doorRail(v: Toption) {
    this._doorRail = v;
    this._reRender();
  }
  ...
  // 門軌選項
  get doorRailOptions() {
    if (this.typhoonProof) {
      return doorRail_antiTyphoon;
    } else {
      return doorRail_normal;
    }
  }
  ...
  // 牌價複價
  get listPriceTotal() {
    return new Decimal(this.qty || 0).mul(this.listPrice.replaceAll(',', '')).toNumber().toLocaleString();
  }
  ...
  // 複價
  get priceTotal() {
    return new Decimal(this.qty || 0).mul(this.unitPrice.replaceAll(',', '')).toNumber().toLocaleString();
  }
...
// =======================================================================
class Class_part {
  constructor(part: Tdata['mainProductArr'][0]['part'][0], parent: Class_mainProduct, reRender: TreRender) {
    this._reRender = reRender;
    this._parent = parent;
  ...
  get qty() {
    if (this._qty) {
      return parseFloat(this._qty).toFixed(2);
    }
...
class Class_payInfo {
  ...
  get tradingLocation() {
    return this._tradingLocation;
  }
  set tradingLocation(v: string) {
    this._tradingLocation = v;
    this._reRender();
  }
  ...
  set deliveryPayment(v: string) {
    this._deliveryPayment = v;
    this._reRender();
  }

  private _installedPayment;
  get installedPayment() {
    return this._installedPayment;
  }
  set installedPayment(v: string) {
    this._installedPayment = v;
    this._reRender();
  }

  private _eleConnectPayment;
  get eleConnectPayment() {
    return this._eleConnectPayment;
  }
  set eleConnectPayment(v: string) {
    this._eleConnectPayment = v;
    this._reRender();
  }
} // Class_payInfo
...
class Class_quotation {
  constructor(reRender: TreRender, data: Tdata, prodCellConfig: TprodCellConfig, partCellConfig: TpartCellConfig) {
    ...
    // 報價單基本資料
    this.classBasicInfo = new Class_basicInfo(reRender, data.basicInfo, data.fakeClienProfile, this);
    // 主產品設定 (包括材料配件設定)
    this.mainProductArr = data.mainProductArr.map((mainProduct) => new Class_mainProduct(mainProduct, reRender));
    //  付款資訊
  ...
  // 營業稅
  get businessTax() {
    return new Decimal(this.subTotal).mul(0.05).toString();
  }
  // 總計
  get total() {
    return new Decimal(this.subTotal).add(this.businessTax).toString();
  }
  // 變更所有mainProduct的折數
...
const useQuotation = (data: Tdata | undefined) => {
  ...
  const checkData = () => {
    if (data) {
      return new Class_quotation(reRender, data, mainProdCellConfigOri(), partCellConfigOri());
    }
...
type TmainProdInputCellType = {
  [key in keyof Pick<Class_mainProduct, 'discount' | 'category' | 'L' | 'W' | 'h' | 'qty' | 'memo'>]: { type: 'input' };
};
type TmainProdSelectCellType = {
  [key in keyof Pick<Class_mainProduct, 'series' | 'B' | 'doorType' | 'material' | 'surface' | 'horsepower'>]: {
    type: 'select';
  };
};
...
type TmainProdCheckboxCellType = {
  [key in keyof Pick<Class_mainProduct, 'ejectionDoor' | 'typhoonProof'>]: { type: 'checkbox' };
};
type TmainProdReadOnlyCellType = {
  [key in keyof Pick<
    Class_mainProduct,
    'listPrice' | 'listPriceTotal' | 'unitPrice' | 'priceTotal' | 'area' | 'cai'
  >]: { type: 'readOnly' };
};

type TprodKeys = keyof (TmainProdInputCellType &
...
type TpartSelectCellType = { [key in keyof Pick<Class_part, 'material'>]: { type: 'select' } };
