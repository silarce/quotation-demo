...
//  捲軸計算 //直徑
const calcReel = (L: number, weight: number) => {
  let reelData = '資料錯誤';
...
//馬力計算
const calcHorsepower = (weight: number) => {
  let horsepower = '資料錯誤';
...
const calcBAndD = (reelData: string, h: number) => {
  let bArray: number[] = [];
  let B = 0;
  let D = 0;
...
export { calcReel, calcHorsepower, calcBAndD };
