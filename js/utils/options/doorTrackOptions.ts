...
export const optionsCre_doorTrack_typhoonProtection = (
  { emptyOption }: { emptyOption?: boolean } = {}
): Toption[] => {
  const optionArr = [
  ...
  if (emptyOption) optionArr.unshift({ value: "", label: "不拘", icon: undefined })
  return optionArr
}

export const optionsCre_doorTrack_normal = (
  { emptyOption }: { emptyOption?: boolean } = {}
): Toption[] => {
  const optionArr = [
    {
