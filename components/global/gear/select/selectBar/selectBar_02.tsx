...
export default function SelectBar_02({
  ...
  const [valueArr, setValueArr] = useState<(Toption | null)[]>(
  ...
  return (
    <div className={classNames(scss.selectBar, className)}>
      {selPropsArr.map((props, index) => {
        const { options, placeholder, defaultValue, value, onChange, wrapperClassName, className, selClassNames, gap } =
          props;
            ...
            <InputSel
              ...
              selectProps={{
                ...
                selClassNames: {
                  ...
                  singleValue: (state) =>
                    classNames(scss.selSingleValue, { [scss[`mr${gap}`]]: gap }, selClassNames?.singleValue?.(state)),
