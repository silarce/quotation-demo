...
// 路由表
import sidePathList from "../SideNav/pathList";
  ...
  return (
    <div className={scss.container}>
      ...
      <Image className={scss.iconMenu}
        src={iconMenu} alt="menu" onClick={() => setIsShowMenu(true)} />
  ...
  return (
    <>
      ...
      >
        {/* close btn */}
        <Image
          className="absolute top-[21px] right-[26px] w-[18px]"
        ...
        <div className={scss.panel}>
          <span className={scss.left} onClick={() => setIsShowChangePw(true)}>變更密碼</span>
        ...
        <Collapse className={scss.navWrapper}
          ghost
          onChange={(v) => { setActivePanel(v as string[]) }}
        >
          <Panel header={<PanelHeader />} key="0"
            className={classNames({ [scss.isActive]: checkIsActive("0") })}
          >
            <PanelBody />
          </Panel>
        </Collapse>
      ...
      <ChangePwPanel
        visible={isShowChangePw}
        onCancel={() => setIsShowChangePw(false)}
      />
    </>
  )
...
const PanelBody = () => {
  ...
  let linkList = sidePathList["/home"]
  ...
  return (
    <div className={classNames(scss.nav, "relative")}>
      {linkList?.list.map((item, index) => {
        ...
        let active = reg.test(asPath) ? scss.active : ""
        ...
        if (list) {
          return (
            <Collapse key={index} className={scss.nav}
