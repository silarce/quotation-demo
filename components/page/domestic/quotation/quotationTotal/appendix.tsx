...
export default function Appendix({ disabled }: { disabled: boolean }) {
  ...
  const preloadImg = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (e: ProgressEvent<FileReader>) => {
      ...
      const fileType = imageReg.test(type) ? 'image' : pdfReg.test(type) ? 'pdf' : 'other';
  ...
  return (
    <div className={`${styleL.appendix} ${styleL.listContainer} `}>
      ...
      {fileList.map((item, index) => {
        ...
        return (
          <div key={index}>
            {disabled ? <span></span> : <IconRemoveCircle onClick={() => removeFile(index)} />}
            <span className={styleL.serialNumber}>
              {/*  eslint-disable-next-line @next/next/no-img-element */}
              <img src={iconAttacth.src} alt="" />
            </span>
            {/* <span className={styleL.serialNumber}>{index + 1}</span> */}
            <span className={styleL.fileName} onClick={() => setShowModal(index)}>
              {fileName}
            </span>
            {fileType === 'pdf' && showModal === index && (
              <PdfViewer01 pdfSrc={fileSrc} fileName={fileName} closeModal={closeModal} />
            )}
            {fileType === 'image' && (
      ...
      <div>
        {disabled ? (
          ...
          <label htmlFor="uploadImg">
            <input id="uploadImg" type="file" style={{ display: 'none' }} onChange={preloadImg} />
