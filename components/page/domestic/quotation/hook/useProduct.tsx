...
import { Tpart, TpartList, TquoteTypeKeys, fakePartGroupOri } from 'fakeDatabase/domestic/quotation/fakeQuotPart';
...
const { keyList: prodKeyList } = pordCellConfig;

// ============================================================
// ============================================================
// ============================================================

function useProduct(productListOri: Tproduct[] | undefined, disabled: boolean) {
...
class ProdClass {
  ...
  get doorRailOptions() {
    if (this.typhoonProof) {
      return optionsGroup.doorRail_antiTyphoon;
    } else {
      return optionsGroup.doorRail_normal;
    }
  }
  ...
  constructor({
    ...
    this.doorType = optionsGroup['doorType'].find((item) => item.value === doorType) ?? unexpectedOption(doorType);

    if (horsepower) {
      this._horsepower =
        optionsGroup['horsepower'].find((item) => item.value === horsepower) ?? unexpectedOption(horsepower);
    } else {
      this._horsepower = { value: 'autoCalc', label: '自動計算' };
    }

    this.quoteType = optionsGroup['quoteType'].find((item) => item.value === quoteType) ?? unexpectedOption(quoteType);
    this.material = optionsGroup['material'].find((item) => item.value === material) ?? unexpectedOption(material);
    this.surface = optionsGroup['surface'].find((item) => item.value === surface) ?? unexpectedOption(surface);

    this.doorRail = optionsGroup['doorRail'].find((item) => item.value === doorRail) ?? unexpectedOption(doorRail);

    this._B = (() => {
      if (B) {
        return unexpectedOption(B);
      } else {
    ...
    // 建立part
    this.part = fakePartGroup[quoteType as TquoteTypeKeys].map(
      (partData) =>
        new PartClass({
          partData,
          setProductList,
  ...
  // L*(h+B) = area
  get area() {
    return new Decimal(this._L || this._W).mul(Decimal.add(this._h, this.B.value)).toFixed(2).toString();
  }
  // area *10.89 = cai 取整數
  get cai() {
  ...
  // --------------
  onInputChange(value: string, key: keyof TproductString) {
    ...
    key = key as Exclude<keyof TproductString, 'area' | 'unitPrice' | 'subTotal' | 'cai' | 'unitWeight'>;
    this[key] = value;
    this.setProductList((state) => [...state]);
  }

  onSelChange(option: Toption | null, key: keyof TproductObject) {
    if (!option) {
      return;
    }
    ...
    if (key === 'quoteType') {
      this.part = fakePartGroup[this.quoteType.value as TquoteTypeKeys].map(
        (partData) =>
          new PartClass({
            partData,
  ...
  onChcekBoxClick(key: keyof TproductBoolean) {
    this[key] = !this[key];
    this.setProductList((state) => [...state]);
  }
...
class PartClass {
  ...
  constructor({
    ...
    this.material = optionsGroup['material'].find((item) => item.value === material) ?? material;
    this.surface = optionsGroup['surface'].find((item) => item.value === surface) ?? surface;
  ...
  // 單價 (有計算折數的單價)
  get price() {
    return Decimal.mul(this.listPrice, this.parent.discount).div(100).toFixed(2).toString();
  }
  //複價 (有計算折數的複價)
  get totalPrice() {
    return Decimal.mul(this.totalListPrice, this.parent.discount).div(100).toFixed(2).toString();
  }

  onSelChange = (option: Toption | null, key: 'material' | 'surface') => {
    if (!option) {
      return;
    }
