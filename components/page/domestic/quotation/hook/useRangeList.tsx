...
export default function useRangeList(quotationData?: Tquotation) {
  let rangeListOri;

  if (quotationData) {
    rangeListOri = quotationData.rangeList;
  } else {
    rangeListOri = fakeEmptyRange;
  }
  ...
  const addRanges = (selRange: Trange[]) => {
    if (!selRange[0]) {
      return ModalInfo02({ title: '請選擇報價範圍' });
    }
