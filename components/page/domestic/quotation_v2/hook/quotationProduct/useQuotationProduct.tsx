...
const useQuotationProduct = ({
  ...
  // 總折數
  const [state_quotationDiscount, _setState_quotationDiscount] = useState<`${number}` | ''>(
    (raw_quotationContent?.discount ?? '') as `${number}` | ''
  );

  const setState_quotationDiscount = (value: `${number}` | '') => {
    _setState_quotationDiscount(value);
...
const calcAndRenewAllProdPrice_sideEffect = ({
  ...
  Object.values(prodDict).forEach((stateProd) => {
    ...
    // prodDict[stateProd.key] = {...stateProd};
...
const calcProdTotalPrice = ({
  ...
  Object.entries(data_accessoryDict).forEach(([key, acce]) => {
    const copy = { ...acce };
    const { quantity, price, dualPrice, unitPrice, totalPrice } = copy;
