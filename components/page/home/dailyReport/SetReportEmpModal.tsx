...
export default function SetReportEmpModal({
  ...
  const [theDataArr, setTheDataArr] = useState<TsetReportEmpModalData[]>(_.cloneDeep(dataArr));
      ...
      <div className={scss.body}>
        {theDataArr.map((data, index) => {
          ...
          if (searchValue) {
            ...
            if (!idNumber.match(regex) && !chName.match(regex) && !job.match(regex) && !`${grade}`.match(regex)) {
              return null;
            }
          }

          return (
            <CellWithBar key={index} className={scss.row} isActive={shouldReport} onClick={onClick}>
