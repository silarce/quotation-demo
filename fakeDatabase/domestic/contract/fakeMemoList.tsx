...
for (i; i <= 30; i++) {
  ...
  const memoDate =
    `111-${`${_.random(1, 12)}`.padStart(2, "0")}-${`${_.random(1, 30)}`.padStart(2, "0")}`
  const belongQuotation =
    `S-110211-${`${_.random(1, 12)}`.padStart(2, "0")}`
...
function randomContent() {
  ...
  const randomKey = _.random(0, 5) as (keyof typeof contents)
