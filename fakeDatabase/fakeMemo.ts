...
const fakeMemoDataArr: TfakeMemoData[] = [
  ...
  },
  {
    ...
    doorType: ["SJ-302"],
    doorForm: ["一般", "防颱"],
    content: "抗風壓結構計算技師簽證費用、實驗室門樘試燒、剖門檢驗、水閘門試水認證、材料檢驗費用、防颱底座鎖固、防颱中柱、高空作業、自動防火連動操作裝置、前遮板、砂利康、懸吊系統、門框補強立柱、收邊料，單價另計。",
  },
  {
    id: 5,
    prodClass: ["防火防煙捲門系列"],
...
const checkData = () => {
  fakeMemoDataArr.forEach((item, index) => {
    if (item.id !== index + 1) {
      throw new Error(
        `fakeMemoData資料的id要等於index-1。${`id:${item.id}`} ${`index:${index}`}   `
      );
    }
  })
}
