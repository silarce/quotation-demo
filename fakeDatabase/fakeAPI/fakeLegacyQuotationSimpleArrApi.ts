...
class Class_fakeApi_legacyProjectSimple {
  get = (param?: {
    ...
    const arr = Object.values(fakeLegacyQuotationDataList).map((item, index) => {
      const clientData = fakeClientProfileList[item.clientId];

      if (filter?.county) {
        if (item.basicInfo.constructionCounty !== filter.county) {
          return;
        }
      }

      if (filter?.clientName) {
        if (!clientData.name.includes(filter.clientName)) {
          return;
        }
      }

      if (filter?.constructionName) {
        if (!item.basicInfo.constructionName.includes(filter.constructionName)) {
          return;
        }
      }
