...
import {
  TlegacyQuotation,
  fakeLegacyQuotationDataList,
  emptyLegacyQuotation,
} from 'fakeDatabase/domestic/_fakeLegacyQuotation';
...
class Class_fakeApi_legacyQuotation {
  ...
  post(quotation: TlegacyQuotation) {
    ...
    if (fakeLegacyQuotationDataList[quotationId]) {
      return alert('此報價單編號已存在');
    }
...
const fakeApi_legacyQuotation_creator = (id: string) => {
  ...
  if (quotation) {
    return new Class_fakeApi_legacyQuotation(quotation);
  }
